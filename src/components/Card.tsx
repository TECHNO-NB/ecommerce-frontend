  ...
  return (
    <div className="mt-0 select-none lg:mt-2">
      <div className="flex justify-center items-center">
        <div className="card bg-[#F6F7FB] rounded-lg shadow-md overflow-hidden w-72 h-[320px] transition-transform duration-300 ease-in-out hover:-translate-y-1">
          {/* Image Container */}
          <div
            onClick={() => navigate(`/productdetails/${data._id}`)}
            className="h-[180px] bg-[#F6F7FB] cursor-pointer overflow-hidden flex items-center justify-center"
          >
            <img
              src={data.image}
              alt="Product"
          ...
          <div className="bg-white p-4 flex flex-col justify-between h-[150px]">
            <div>
              <div className="w-full flex  justify-between">
                ...
                <h3 className="hidden sm:block text-[7px] md:text-lg text-center  items-center justify-center bg-orange-600 rounded-3xl text-white px-1 md:px-2 font-bold mb-2">
                  20% off
                </h3>
              </div>
              {/* Price with Star */}
            ...
            {user.isLoggedIn && user.role !== "admin" ? (
              <button
                ...
                className="inline-block bg-pink-500 text-white px-5 py-2 rounded-md text-sm font-bold text-center transition-colors duration-300 ease-in-out hover:bg-blue-700"
              >
                Add to Cart
              </button>
            ) : (
